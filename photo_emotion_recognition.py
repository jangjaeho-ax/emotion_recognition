...
def emotion_recognition(image_path):
    ...
    if results.multi_face_landmarks:
        ...
        # print(rc_dict)
        #test = mf.get_side_face(rc_dict,yaw)
        #mf.show_scatter2(test)
        landmark2D = mf.get_side_face(rc_dict, yaw)
        mf.show_scatter2(rotated_coords)
        ...
        cv2.putText(image, 'emotion : '+ emotions[predicted_labels[0]], (20, 450), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 255, 0), 2)
        end = time.time()
        totalTime = end - start
        cv2.waitKey()
