...
def emotion_recognition():
    ...
    while cap.isOpened():
        ...
        if results.multi_face_landmarks:
            ...
            if yaw > 2 or yaw < -2:
                mf.rotateY(rotated_coords, center_point, yaw - math.pi * 0.5)
            elif yaw > 1 or yaw < -1:
                mf.rotateY(rotated_coords, center_point, -yaw * 0.7)
            elif yaw < 0.1 and yaw > - 0.1:
                mf.rotateY(rotated_coords, center_point, -yaw * 3)
            else:
                mf.rotateY(rotated_coords, center_point, -yaw * 1.2)
            '''
            center_point = (int(xcenter), int(ycenter), int(((xcenter - img_w / 2) ** 2 + (ycenter - img_h / 2) ** 2) ** .4))
            ...
            totalTime = end - start
            cv2.putText(image, "pitch: " + str(pitch), (400, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255),
                        2)
            cv2.putText(image, "yaw: " + str(yaw), (400, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255),
                        2)
            cv2.putText(image, "roll: " + str(roll), (400, 150), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255),
                        2)
